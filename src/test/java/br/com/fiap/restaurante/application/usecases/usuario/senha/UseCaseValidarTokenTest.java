...
public class UseCaseValidarTokenTest {
    ...
    @Test
    void deveRetornarFalseSeTokenForNull() {
        when(jwtService.validarToken(null)).thenReturn(false);

        boolean resultado = useCase.run(null);

        assertThat(resultado).isFalse();
        verify(jwtService, times(1)).validarToken(null);
    }
