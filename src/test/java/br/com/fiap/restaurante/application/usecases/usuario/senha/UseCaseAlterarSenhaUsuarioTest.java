...
public class UseCaseAlterarSenhaUsuarioTest {
    ...
    void deveAlterarSenhaComSucesso() {
        Long id = 1L;
        String senhaAntiga = "mockAntiga123!";
        String senhaNova = "mockNovaSenha456@";
    ...
    void deveFalharSeSenhaAntigaEstiverIncorreta() {
        ...
        Usuario usuario = Helper.gerarUsuario();
        usuario.setId(id);
        usuario.setSenha("senhaCertaMockada!");

        when(gateway.buscarUsuarioPorIdentificador(id)).thenReturn(usuario);
        when(passwordEncoder.matches("senhaErradaMock", "senhaCertaMockada!")).thenReturn(false);

        assertThatThrownBy(() -> useCase.run(id, "senhaErradaMock", "mockNovaSenha456@"))
    ...
    void deveFalharSeSenhaNovaForNula() {
        ...
        Usuario usuario = Helper.gerarUsuario();
        usuario.setId(id);
        usuario.setSenha("mockAntiga123!");

        when(gateway.buscarUsuarioPorIdentificador(id)).thenReturn(usuario);
        when(passwordEncoder.matches("mockAntiga123!", "mockAntiga123!")).thenReturn(true);

        assertThatThrownBy(() -> useCase.run(id, "mockAntiga123!", null))
                .isInstanceOf(ValidationException.class)
    ...
    void deveFalharSeNaoTiverLetraMaiuscula() {
        assertSenhaFraca("mocksenha@123", "Senha deve conter pelo menos uma letra maiuscula!");
    }

    @Test
    void deveFalharSeNaoTiverNumero() {
        assertSenhaFraca("Senha@mock", "Senha deve conter pelo menos um número.");
    }
    ...
    void deveFalharSeUsuarioNaoForEncontrado() {
        ...
        assertThatThrownBy(() -> useCase.run(id, "qualquerSenha", "mockNovaSenha456@"))
                .isInstanceOf(ValidationException.class)
    ...
    void assertSenhaFraca(String senhaNova, String mensagemEsperada) {
        ...
        assertThatThrownBy(() -> useCase.run(id, "mockAntiga123!", senhaNova))
                .isInstanceOf(ValidationException.class)
